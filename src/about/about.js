...
const loadEmployeeDetails = (id, modal, employee_el) => {
  ...
  employee_el.querySelector(
    ".employee-name"
  ).textContent = `${teamData[id].name}`;
  employee_el.querySelector(
    ".employee-occupation"
  ).textContent = `${teamData[id].occupation}`;
  employee_el.querySelector(
    ".employee-img"
  ).style.backgroundImage = `url('./assets/team/${teamData[id].imageURL}')`;
};
...
const loadEmployee = () => {
  return Object.entries(teamData).map(([k, v], i) => {
